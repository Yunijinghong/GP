...
data = pd.read_excel(filename, skiprows=1, nrows=79, usecols=[0, 4, 5])  # 第1列为x，第2列和第6列为y

# 提取x轴和y轴数据
x = data.iloc[:, 0].values  # x 轴数据
y1 = data.iloc[:, 1].values  # y1 为第2列数据
y2 = data.iloc[:, 2].values  # y2 为第6列数据
...
# 绘制 y1 数据
plt.scatter(x, y1, label='MA', color='blue')  # 2.0V
plt.plot(x_fine, cs1(x_fine), label='MA', color='cyan')

# 标注 y1 的峰和谷
if not pd.isna(results['y1_peaks']).all():
    plt.scatter(results['y1_peaks'], cs1(results['y1_peaks']), color='red', label='MA Peaks')
if not pd.isna(results['y1_valleys']).all():
    plt.scatter(results['y1_valleys'], cs1(results['y1_valleys']), color='green', label='MA Valleys')

# 绘制 y2 数据
plt.scatter(x, y2, label='VO3S', color='orange')  # 第6列数据
plt.plot(x_fine, cs2(x_fine), label='VO3S', color='yellow')

# 标注 y2 的峰和谷
if not pd.isna(results['y2_peaks']).all():
    plt.scatter(results['y2_peaks'], cs2(results['y2_peaks']), color='red', label='VO3S Peaks')
if not pd.isna(results['y2_valleys']).all():
    plt.scatter(results['y2_valleys'], cs2(results['y2_valleys']), color='green', label='VO3S Valleys')
